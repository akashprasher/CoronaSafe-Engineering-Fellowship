...
def list_todo():
    if path.exists("todo.txt"):  # checking for the todo file
        ...
        for i in range(1, number_of_lines + 1):
            current_todo = "[" + str(index_inverse) + \
                "] " + final_list[i] + "\n"
            # Printing using UTF-8, using print() directly giving unwated symbols as result
            sys.stdout.buffer.write(current_todo.encode('utf8'))
            index_inverse -= 1
        todo_file.close
    else:
